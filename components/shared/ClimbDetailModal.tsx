...
import CompareWithMinePicker, {
  type MyRunSummary,
} from "@/components/compare/CompareWithMinePicker";
...
export default function ClimbDetailModal({ climb, onClose }: ClimbDetailModalProps) {
  ...
  // Whether this climb belongs to another user — the key is scoped
  // RouteData/{ownerUserId}/…, so the owner is the second path segment. Only
  // then does cross-user "compare with mine" apply (comparing to your own run
  // is already the in-console rail flow).
  const owner = climb.key.split("/")[1] ?? "";
  const isOtherUsersClimb = Boolean(user) && owner !== "" && owner !== user!.uid;

  // Pick one of my runs → host the console on MY route with the guest climb
  // added as a second slot. Mixed-owner keys already ride in the `keys` CSV.
  const onPickMyRun = (run: MyRunSummary) => {
    if (!user) return;
    const url = buildRouteUrl(
      user.uid,
      { state: run.state, area: run.area, route: run.route },
      { keys: [run.key, climb.key], mode: "multiple" },
    );
    setShowPicker(false);
    go(url);
  };
  ...
  return (
    <>
      {showPicker && isOtherUsersClimb && user && (
        <CompareWithMinePicker
          myUserId={user.uid}
          guest={{ state: climb.state, area: climb.area, route: climb.route }}
      ...
      >
        ...
        <div className="px-5 py-4">
          ...
          {climb.notes && <p className="mt-3 whitespace-pre-wrap text-sm text-fg">{climb.notes}</p>}
          ...
          <div className="mt-4 flex flex-col gap-2">
            ...
            {/* Cross-user: overlay this climb against one of my own runs. Only
              shown for another user's climb — comparing my own runs is the
              in-console rail flow. */}
            {isOtherUsersClimb && (
              <button
                type="button"
                onClick={() => setShowPicker(true)}
                className="flex w-full items-center justify-center gap-2 rounded-md border border-edge/60 px-4 py-2.5 text-sm font-semibold text-fg transition hover:bg-inset/60"
              >
                <svg
                  className="h-4 w-4 shrink-0"
                  fill="none"
