...
- **Landmark Flips** — MediaPipe mislabels the Climber's left/right sides for a
  ...
  One-Euro smoother filters each keypoint *by name independently*, so it cannot
...
- **Missing landmarks / connectors** — three gates erase joints: the
  ...
  in *both* anchors, and edges drawing only when both endpoints exist.
- **Jittery vs robotic overlay** — a single uniform `frameStep` couples two
  unrelated problems: per-detection *noise* (jitter) and *under-sampling* of
...
2. **Flip detection (stateful walk forward).** Walk detected frames in order,
   comparing each to the previous *accepted* frame. A **Landmark Flip** is a
   fast, discontinuous sign-change in shoulder/hip separation where each labelled
   torso joint *teleports* (the swapped assignment would have low displacement) —
...
3. **Adaptive Refinement (unified).** A second pass densely re-samples
   *frame-by-frame, no stride skip* only the segments that need it: high
...
3. **Uniform dense stride + zero-phase smoothing only** — simpler and
   predictable, but to capture one fast move it must lower the *global* stride,
...
5. **Per-lighting-mode preprocessing handlers** — rejected for now: highest
   complexity and a real risk of *degrading* a model trained on natural images;
...
- **The pass order is now load-bearing.** Flip detection must run on the sparse
  detected frames *before* interpolation — interpolating across an uncorrected
...
- **Flip detection is sampling-coupled.** At Fast tier (large `frameStep`) a
  genuinely fast rotation can complete inside one gap and *look* like a flip; the
