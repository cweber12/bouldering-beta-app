...
describe("buildCompareUrl", () => {
  it("encodes a single key", () => {
    expect(buildCompareUrl("RouteData/u/x.json")).toBe(
      "/compare?keys=RouteData%2Fu%2Fx.json",
    );
  });
  ...
  it("appends route context in order", () => {
    expect(
      buildCompareUrl("a", { state: "CO", area: "RMNP", route: "Slab" }),
    ).toBe("/compare?keys=a&state=CO&area=RMNP&route=Slab");
  });

  it("emits mode only when supplied", () => {
    expect(buildCompareUrl("a")).not.toContain("mode=");
    expect(buildCompareUrl("a", { mode: "single" })).toBe(
      "/compare?keys=a&mode=single",
    );
