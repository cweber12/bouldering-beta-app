...
const orbSummary = { refKeypointCount: 480, keyframeCount: 0, keyframeKeypoints: { min: 0, avg: 0, max: 0 } };
...
const referenceFrameMeta = { width: 720, height: 1280, refKeypointCount: 480 } as unknown as ReferenceFrameMeta;

describe("buildHarnessPayloads", () => {
  it("wraps the diagnostics + frames as the pose half", () => {
    const { pose } = buildHarnessPayloads({ diagnostics, frames, referenceFrameMeta, setupHash: "hash1" });
  ...
  it("puts extraction data + attribution in the orb half", () => {
    const { orb } = buildHarnessPayloads({ diagnostics, frames, referenceFrameMeta, setupHash: "hash1" });
  ...
  it("tolerates a null reference frame meta", () => {
    const { orb } = buildHarnessPayloads({ diagnostics, frames, referenceFrameMeta: null, setupHash: "h" });
