...
- **Square-bounded pattern** (scan flow Steps 2 & 3): bounds the media's *longer* edge to the available vertical space `s` (an `s × s` square) so portrait fills the height and landscape never sprawls. The only difference from the viewport-fit width term is `min(1, ratio)` instead of `ratio`. Helpers in `utils/mediaContainerStyle.ts`: `squareMediaStyle(w, h, s)` / `squareMediaWidth(w, h, s)` take a **measured** `s` (px, via `useMeasuredHeight` on a `flex-1 min-h-0` stage); `squareMediaMaxWidth(w, h, offset)` is the dvh-calc variant for flow layouts. The scan video stage is flush (no border/radius/padding) and centered on `bg-surface`; the transport bar aligns to `squareMediaWidth`. Default the pre-load aspect ratio to portrait `{ w: 9, h: 16 }` (ascents are recorded vertically).
