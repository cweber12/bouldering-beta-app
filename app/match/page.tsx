    ...
    <div className="mx-auto w-full max-w-3xl px-6 py-10 flex flex-col gap-8">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex flex-col gap-1">
          <h1 className="text-2xl font-bold tracking-tight text-[#F5FBE6]">Route Matching</h1>
        ...
        <Link href="/upload" className="shrink-0 text-xs text-[#6a9ca0] hover:text-[#c5dcd8] transition">
          ← Back to upload
        </Link>
      </div>
      ...
      {/* Climb data section */}
      <div className="rounded-lg border border-[#2d4e5e] bg-[#233D4D] px-5 py-4 flex flex-col gap-4">
        <p className="text-sm font-medium text-[#c5dcd8]">Climbs</p>

        {hasAttempt && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between rounded-lg border border-[#2d4e5e] bg-[#192e3a] px-4 py-3">
              <div className="flex flex-col gap-0.5">
                <span className="flex items-center gap-2 text-xs font-mono text-[#c5dcd8]">
                  {attempt.id}
                  <span className={[
                  ...
                  {attempt.rating && (
                    <span className="rounded px-1.5 py-0.5 text-xs font-medium font-sans bg-[#233D4D] text-[#c5dcd8]">
                      {attempt.rating}
                    </span>
                  )}
                </span>
                <span className="text-xs text-[#6a9ca0]">
            ...
            {attempt.notes && (
              <div className="rounded-lg border border-[#2d4e5e] bg-[#192e3a]/50 px-4 py-2.5">
      ...
      {hasAttempt && (
        <div className="rounded-lg border border-[#2d4e5e] bg-[#233D4D] px-5 py-4 flex flex-col gap-3">
          <button
            onClick={() => setStyleOpen(o => !o)}
            className="flex items-center justify-between w-full text-left"
            aria-expanded={styleOpen}
          >
            <span className="text-sm font-medium text-[#c5dcd8]">Skeleton style</span>
            <svg
              className={["h-4 w-4 text-[#6a9ca0] transition-transform duration-200", styleOpen ? "rotate-180" : ""].join(" ")}
          ...
          {styleOpen && (
            <div className="flex flex-col gap-4 pt-1">
              <p className="text-xs text-[#6a9ca0] -mt-1">
                Adjust colours and sizes — changes apply to the player in real time.
              </p>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-[#8fbfc0]">Limb color</label>
                  <input
                    type="color"
                    value={skeletonStyle.limbColor?.replace(/rgba?\([^)]+\)/, "#00dc78") ?? "#00dc78"}
                    onChange={e => setSkeletonStyle(s => ({ ...s, limbColor: e.target.value }))}
                    className="h-8 w-full cursor-pointer rounded border border-[#2d4e5e] bg-[#192e3a] p-0.5"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs text-[#8fbfc0]">Joint color</label>
                  <input
                    type="color"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="flex justify-between text-xs text-[#8fbfc0]">
                    <span>Line width</span>
                    <span className="font-mono text-[#c5dcd8]">{skeletonStyle.lineWidth ?? 2.5}px</span>
                  </label>
                  <input
                    type="range" min={0.5} max={8} step={0.5}
                    value={skeletonStyle.lineWidth ?? 2.5}
                    onChange={e => setSkeletonStyle(s => ({ ...s, lineWidth: Number(e.target.value) }))}
                    className="accent-[#FE7F2D]"
                  />
                </div>
                <div className="flex flex-col gap-1.5">
                  <label className="flex justify-between text-xs text-[#8fbfc0]">
                    <span>Point radius</span>
                    <span className="font-mono text-[#c5dcd8]">{skeletonStyle.pointRadius ?? 5}px</span>
                  </label>
                  <input
          ...
          <label
            className={[
              ...
              !hasAttempt || isMatching
                ...
                : [
                    "bg-[#215E61] border-[#2d4e5e] text-[#c5dcd8]",
                    "hover:border-[#FE7F2D]/60 hover:text-[#F5FBE6]",
                    hasAttempt && !imageFile && !isMatching ? "animate-pulse border-[#FE7F2D]/30" : "",
                  ].join(" "),
            ].join(" ")}
          >
            ...
            <span className="font-medium text-[#F5FBE6]">{isMatching ? "Loading..." : "Select a photo"}</span>
          ...
          <button
            ...
            className={[
              ...
              !hasAttempt || isMatching
                ? "cursor-not-allowed border-[#192e3a] opacity-40 text-[#8fbfc0]"
                : [
                    "cursor-pointer bg-[#215E61] border-[#2d4e5e] text-[#c5dcd8]",
                    "hover:border-[#FE7F2D]/60 hover:text-[#F5FBE6]",
                    hasAttempt && !imageFile && !isMatching ? "animate-pulse border-[#FE7F2D]/30" : "",
                  ].join(" "),
            ].join(" ")}
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="relative w-full">
              ...
              <img
                ...
                className="max-h-80 w-full rounded-xl border border-[#2d4e5e] bg-[#233D4D] object-contain"
              />
              <CropBoxOverlay
                box={imageCrop}
            ...
            <button
              ...
              disabled={!hasAttempt}
              className="flex items-center justify-center gap-2 rounded-xl bg-[#FE7F2D] px-6 py-3 text-sm font-semibold text-[#F5FBE6] transition hover:bg-[#e56015] disabled:cursor-not-allowed disabled:opacity-50 ring-2 ring-[#FE7F2D]/30 ring-offset-2 ring-offset-[#0f1c24] animate-pulse"
            >
              Apply &amp; View
            </button>
          </div>
      ...
      {isMatchDone && matchResult && (
        <div className="rounded-lg border border-[#2d4e5e] bg-[#233D4D] px-5 py-4 flex flex-col gap-1">
          <p className="text-sm font-medium text-[#c5dcd8]">View statistics</p>
          <div className="mt-2 grid grid-cols-3 gap-4 text-center">
            <div>
              <p className="text-xl font-bold text-[#F5FBE6]">{matchResult.matches.length}</p>
              <p className="text-xs text-[#6a9ca0]">good matches</p>
            </div>
            <div>
              <p className="text-xl font-bold text-[#F5FBE6]">{matchResult.queryKeypoints}</p>
              <p className="text-xs text-[#6a9ca0]">query keypoints</p>
            </div>
            <div>
              <p className="text-xl font-bold text-[#F5FBE6]">{matchResult.referenceKeypoints}</p>
              <p className="text-xs text-[#6a9ca0]">reference keypoints</p>
            </div>
          </div>
          ...
          {exportStatus === "rendering" ? (
            <div className="flex flex-col gap-2">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-[#192e3a]">
                <div
                  className="h-full rounded-full bg-[#FE7F2D] transition-all duration-150"
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 rounded-xl border border-[#2d4e5e] bg-[#233D4D] px-6 py-3 text-sm text-[#c5dcd8] transition hover:border-[#3d6474] hover:text-[#F5FBE6]"
            >
